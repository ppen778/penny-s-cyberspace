(function () {
  ...
  const DEFAULT_FONT = 'bold 30px "Microsoft YaHei","PingFang SC",sans-serif';
  ...
  function createTextTexture(gl, text, font = DEFAULT_FONT, color = '#2A2A2A') {
  ...
  class Title {
    constructor({ gl, plane, text, textColor = '#2A2A2A', font = DEFAULT_FONT }) {
      autoBind(this);
      this.gl = gl;
      this.plane = plane;
      this.text = text;
  ...
  class App {
    constructor(container, { items, bend = 3, textColor = '#2A2A2A', borderRadius = 0.05, font = DEFAULT_FONT, scrollSpeed = 2, scrollEase = 0.05 } = {}) {
    ...
    addEventListeners() {
      ...
      if (this.container) this.container.addEventListener('keydown', this.boundOnKeyDown);
    }
    destroy() {
  ...
  window.initCircularGallery = (container, items, options) => new App(container, { items, ...options });
})();
